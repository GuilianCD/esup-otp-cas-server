...
/** @typedef {{ if_IP_in?: string[], if_service?: { regexp?: string }, if_not_service?: { regexp?: string } }} conds */
...
function compute_no_otp_conds(req, conds, msg) {
    ...
    if (req.query.service) {
        if (conds.if_service?.regexp) {
            if (req.query.service.match(conds.if_service.regexp)) {
                console.log(msg + " because of conds.if_service.regexp for service", req.query.service)
                return true
            }
        }
        if (conds.if_not_service?.regexp) {
            if (!req.query.service.match(conds.if_not_service?.regexp)) {
                console.log(msg + " because of conds.if_not_service.regexp for service", req.query.service)
                return true
            }
        }
    }
