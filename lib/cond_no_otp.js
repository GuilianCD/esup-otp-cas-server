...
async function no_otp(req) {
    if (await compute_no_otp_conds(req, conf.no_otp, 'no OTP')) {
        return true
    }
    if (await compute_no_otp_conds(req, conf.no_otp?.if_not_activated_for_user_and || {}, 'no OTP if not activated for user')) {
