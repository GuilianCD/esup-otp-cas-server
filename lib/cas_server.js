...
/** @param {session_but_not_validated & session_validated} session
  * @param {string} service
  * @param {string} ticket */
async function get_logged_user_in_session(session, service, ticket, addServiceHashtoPath) {
        ...
        if (!get_valid_uid(req.session)) {
            if (req.query.gateway) {
                ...
                try {
                    ...
                    await get_logged_user_in_session(req.session, service, ticket, shouldAddServiceHashtoPath(req));
                } catch (err) {
                    console.error(err)
