...
const casv2_validate_ticket = (query_without_ticket, ticket) => (
    new Promise((resolve, reject) => {
        const url = conf.cas_server_validating_password_base_url + '/serviceValidate?service=' + encodeURIComponent(ourUrl(query_without_ticket)) + '&ticket=' + ticket
        request({ url }, function(error, response, body) {
            if (error) {
                console.error("error1")
            } else if (response.statusCode == 200 && body) {
...
function require_a_ticket(query, res) {
    const redirectURL = conf.cas_server_validating_password_base_url + '/login?service=' + encodeURIComponent(ourUrl(query));
