...
const get_validated_ticket = (sessionStore, ticket, service) => (
...
const casv1_validate_ticket = (query_without_ticket, ticket) => (
    new Promise((resolve, reject) => {
        const url = conf.cas_server_validating_password_base_url + '/validate?service=' + encodeURIComponent(ourUrl(query_without_ticket)) + '&ticket=' + ticket
...
const casv2_validate_ticket = (query_without_ticket, ticket) => (
    new Promise((resolve, reject) => {
        const url = conf.cas_server_validating_password_base_url + '/serviceValidate?service=' + encodeURIComponent(ourUrl(query_without_ticket) + '&auth_checked') + '&ticket=' + ticket
        request({ url }, function(error, response, body) {
            if (error) {
                console.error("error1")
            } else if (response.statusCode == 200 && body) {
...
async function get_logged_user_in_session(req, ticket) {
    const user_and_ = await casv2_validate_ticket(req.query, ticket);
    console.log("used ticket", ticket, "to know the logged user. Saving ticket and response ");
    req.session.uid = user_and_.uid;
    if (!req.session.ticket_to_v2_raw_response)
        req.session.ticket_to_v2_raw_response = {};
    req.session.ticket_to_v2_raw_response[ticket] = user_and_.raw_response;
}

function redirect_login_on_err(req, res) {
    delete req.query.ask
    req.session.error = "La session avait expiré, veuillez recommencer" // ??
    res.redirect(ourUrl(req.query))
}

function require_a_ticket(query, res, gateway) {
    const redirectURL = conf.cas_server_validating_password_base_url + '/login?service=' + encodeURIComponent(ourUrl(query) + '&auth_checked') + (gateway ? '&gateway=true' : '');
    return res.redirect(redirectURL);
}

function onLoginSuccess(req, res, ticket, gateway) {
    if (!ticket) {
        return require_a_ticket(req.query, res, gateway);
    }
...
function routing() {
    ...
    router.get('/login', async function(req, res) {
        ...
        const ticket = h.getAndDelete(req.query, 'ticket')
        const gateway = h.getAndDelete(req.query, 'gateway')
        const auth_checked = h.getAndDelete(req.query, 'auth_checked') !== undefined
        if (!req.session.uid) {
            if (gateway) {
                return res.redirect(req.query.service) // no way
            } else if (!ticket) {
                return require_a_ticket(req.query, res);
            } else {
                try {
                    await get_logged_user_in_session(req, ticket);
                } catch (err) {
                    console.error(err)
        ...
        try {
            if (req.query.ask == 'rememberMe') {
                ...
                if (!ticket && auth_checked) {
                    return res.redirect(req.query.service) // back from password CAS with no ticket => it must be CAS gateway
                }
                onLoginSuccess(req, res, ticket, gateway)
            } else {
    ...
    router.post('/login', function(req, res) {
        if (req.body.rememberMe && req.session.validated_otp) {
            ...
            verifyOtp(uid, req.body.token).then(() => {
                ...
                res.redirect(ourUrl({ ask: 'rememberMe', ...req.query, ticket: req.body.ticket }))
            }).catch(err => login_page(res, uid, req.body.ticket, err))
        } else {
            redirect_login_on_err(req, res)
        }
    })
    ...
    router.get('/validate', async function (req, res) {
        try {
            const ticket = h.getAndDelete(req.query, 'ticket')
            const ticket_info = await get_validated_ticket(req.sessionStore, ticket, req.query.service)
            if (!ticket_info.v2_raw_response) {
                console.log("proxying ticket validation v1", ticket)
                const user_and_ = await casv1_validate_ticket(req.query, ticket)
    ...
    router.get('/serviceValidate', async function (req, res) {
        ...
        try {
            const ticket = h.getAndDelete(req.query, 'ticket')
            req.query.service = cleanup_service_url(req.query.service) // we must simplify service URL before comparison & before sending to cas_server_validating_password (because it will be encoded => not ignored)
            const ticket_info = await get_validated_ticket(req.sessionStore, ticket, req.query.service)
            if (!ticket_info.v2_raw_response) {
