...
/** @param {string} service
  * @param {string} ticket
  * @returns {Promise<{ uid: string, raw_response: string, date: number}>} */
...
async function get_logged_user_in_session(session, service, ticket) {
    ...
    session.ticket_to_v2_response[ticket] = user_and_
}
...
function onLoginSuccess(req, res, ticket) {
    ...
    let ticket_info = { 
        ...
        v2_response: h.getAndDelete(req.session.ticket_to_v2_response || {}, ticket),
        date: Date.now(),
    }
...
async function validate_ticket(req, cas_validate_ticket) {
    ...
    if (!ticket_info.v2_response) {
        ...
        const user_and_ = await cas_validate_ticket(service, ticket);
        if (user_and_.uid !== ticket_info.uid)
            throw "weird...";
        if (cas_validate_ticket.name.match(/v2/)) {
            ticket_info.v2_response = user_and_;
        }
    }
    return ticket_info;
}
